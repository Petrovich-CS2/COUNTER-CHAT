...
• 🎯 **Режим тренировки** — лёгкий аналог интервального повторения: 10 карточек за
сессию, самооценка «знал / не знал» через кнопки или смахиванием карточки в
сторону — как в приложениях знакомств. Фразы, которые даются сложнее, начинают
появляться заметно чаще, уже освоенные — реже.    
• 🔔 **Напоминание о серии дней** — настоящие push-уведомления (через OneSignal),
приходят даже когда приложение закрыто. Плюс запасной вариант, работающий уже
сейчас в любом браузере без всякой настройки: короткая реакция при каждом
открытии приложения — поощрение за продолженную серию или мягкая поддержка
после перерыва.  
...
• 🔎 **Зум карты** — полноэкранный просмотр схемы с масштабированием до 300% и прокруткой для детального изучения позиций.  
...
• 🏆 **Профиль и достижения** — счётчик дней подряд, 16 достижений с прогресс-барами
(включая 3 секретных, которые не раскрывают условие заранее — только «???» до тех
пор, пока не найдёшь сам), топ-5 самых часто используемых фраз.  
...
1. Проще всего — открыть готовую версию на GitHub Pages: <https://petrovich-cs2.github.io/COUNTER-CHAT/>

2. Либо скачать репозиторий к себе на компьютер (Code → Download ZIP, или `git clone`)
   и открыть `index.html` оттуда в браузере — сайт статический, работает и без
   интернета, всё уже внутри одного файла. *(Просто открыть `index.html` прямо
   на сайте GitHub, кликнув по нему в списке файлов, не сработает — GitHub в
   таком случае покажет исходный код как текст, а не запустит саму страницу.)*

### Установка как приложение

**iOS:** открой сайт в Safari → «Поделиться» → «На экран «Домой»  
**Android:** открой сайт в Chrome → меню (⋮) → «Установить приложение», либо нажми
кастомную кнопку установки прямо в шапке приложения (появляется, когда браузер
поддерживает такую установку).
...
Долгий тап по иконке на домашнем экране (Android) открывает быстрые ярлыки — «Избранное» и «Карты».
...
├── assets/
│   ├── icons/                # иконки манифеста (Android/иконка на главном экране)
│   └── splash/               # загрузочные экраны под все актуальные размеры iPhone
├── tools/                    # вспомогательные инструменты для расстановки точек на картах
│                             # (не часть самого приложения, используются только при разработке)
├── docs/                     # архивные и вспомогательные материалы (гайд, старые баннеры)
...
**Обнаружение обновлений может занимать до ~10 минут после публикации** — GitHub Pages по умолчанию отдаёт все файлы с заголовком Cache-Control: max-age=600, и это касается в том числе системной, полностью браузерной проверки обновления самого sw.js (registration.update()) — мы не можем повлиять на этот конкретный запрос из своего кода, поскольку он выполняется браузером напрямую, в обход нашего JavaScript. Проверка обновления при каждом возврате в приложение гарантирует, что баннер рано или поздно появится — просто не обязательно сразу после публикации.  

**iOS в standalone-режиме (запуск с домашнего экрана) не всегда ведёт себя как обычная вкладка Safari** — сама ОС может «замораживать» уже открытое веб-приложение при сворачивании, и при следующем открытии просто восстанавливать его из памяти, а не запускать заново по-настоящему — из-за этого некоторые события браузера (load, иногда даже pageshow) могут не срабатывать в отдельных сессиях, хотя сам JS-код продолжает работать корректно. Регистрация Service Worker в коде намеренно не полагается на эти события и запускается сразу же, без ожидания.
