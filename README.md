...
сессию, самооценка «знал / не знал» вместо ввода текста. Фразы, которые даются
сложнее, начинают появляться заметно чаще, уже освоенные — реже.    
• 🔔 **Напоминание о серии дней** — переключатель в профиле пробует поставить
настоящее локальное уведомление на вечер через экспериментальный браузерный API
(честно предупреждаем в интерфейсе, если он недоступен — сейчас так практически
везде). Плюс надёжный запасной вариант, работающий уже сейчас в любом браузере:
короткая реакция при каждом открытии приложения — поощрение за продолженную
серию или мягкая поддержка после перерыва.  
