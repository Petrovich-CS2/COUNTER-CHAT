...
• ⭐ **Избранное с экспортом/импортом** — сохраняй часто используемые фразы, чтобы они всегда под рукой; можно перенести список на другое устройство через код-бэкап (данные «живут» в `localStorage`, поэтому без экспорта пропадут при смене телефона или очистке кэша).   
...
•	Вибрация на iOS — navigator.vibrate() не поддерживается в Safari ни в каком виде, включая PWA — Apple принципиально не даёт веб-страницам доступ к вибромотору.  
На Android ощущается, на iPhone просто тихо не срабатывает без ошибок. 
